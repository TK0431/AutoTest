...
namespace AutoTest.Logic
{
    public class P201Logic
    {
        ...
        public void ReadFile(P201ViewModel model)
        {
            ...
            //                        Range = shTemp.Cells[row, col + 3].Text,
            //                    };
            //                    colEvents.Add(tempEvent);

            //                    if (!string.IsNullOrWhiteSpace(tempEvent.Range))
            //                        model.WebElements.Add(tempEvent.Range);

            //                    LogUtility.WriteInfo($"【{shName}-{webId}】Sheet读取数据【{tempEvent.Id}-{tempEvent.Event}】");
            //                }
            //            }
        ...
        private void DoSelenium(P201ViewModel model, string continuEvent = null)
        {
            ...
            //    // 输出路径
            //    _su.OutPath = arg2.Replace("-", "") + "_" + _su.OutPath;
            //    model.OutPath = Environment.CurrentDirectory + @"\" + _su.OutPath;
            //}
            //else
            //{
